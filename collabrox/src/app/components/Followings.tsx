...
import { useGetFollowingsOfPersonQuery } from "../../apis/followApi";
...
const Followings: React.FC = () => {
    ...
    const { data: response, isLoading } = useGetFollowingsOfPersonQuery({
        username: username!,
        sortBy: sortBy.join(','),
        cursor: cursor,
        limit: "50"
    ...
    useEffect(() => {
        ...
        setSearchParams({ open: "followings", sortBy: sortBy.length > 0 ? sortBy.join(',') : 'popular' }, { replace: true });
    }, [sortBy]);
        ...
        <QuickBox allowScroll={false} showTopBar={false}>
            {isAuthenticated ?
                ...
                >
                    ...
                    <div className="sticky bg-white top-0 left-0 w-full flex justify-between items-center px-4 py-2 shadow-sm border-b border-gray-300">
                        <Typography variant="subtitle" className="font-[600]">
                            {isSelfProfile ? "Your" : `${name?.split(" ")[0]}'s`} followings
                        </Typography>
                        { }
                ...
                <div className="flex items-center justify-center mt-4">
                    <Typography variant="subtitle">You must login to see followings</Typography>
                </div>
            }
        </QuickBox>
    );

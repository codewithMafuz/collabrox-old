...
const checkValidation = (
    ...
    // --- undefine or null values checks
    for (let prop in propertyAndValues) {
        if (propertyAndValues[prop] === undefined || propertyAndValues[prop] === null) errors[prop] = true;
    }
    if (Object.keys(errors).length !== 0) return { errors };

    // ---- Filter and merge custom validators with permanent ones
    ...
    // --- Preprocess validators into direct and pattern-based with regex
